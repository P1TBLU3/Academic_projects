...
n = 0:1:50;
d = 1;            % Longitud de la secuencia DSSS
Kd = length(d) - 1; % Longitud de retardo para la convolución
Var = [0 1];      % Vector de dos niveles de varianza para el canal ruidoso

% CANAL IDEAL
Pe_UsA = zeros(2, 4);  % Matriz para almacenar la probabilidad de error de símbolo para Usuario A
Pe_UsB = zeros(2, 4);  % Matriz para almacenar la probabilidad de error de símbolo para Usuario B
BER_UsA = zeros(2, 4); % Matriz para almacenar la tasa de error de bits para Usuario A
BER_UsB = zeros(2, 4); % Matriz para almacenar la tasa de error de bits para Usuario B
P_escalar = zeros(1, 4); % Vector para almacenar el producto escalar de las secuencias DSSS

X_UsA = [x1; x1; x2; x2]; % Matriz de secuencias DSSS para Usuario A
X_UsB = [x2; x3; x3; x2]; % Matriz de secuencias DSSS para Usuario B
